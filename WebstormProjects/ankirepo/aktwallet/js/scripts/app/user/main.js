    ...
    axios.post('http://localhost:3000/api/user/login', {
        ...
        password: pwd
            ...
            if(response.data.status === true) {
                ...
                // $(location).attr('href', 'http://localhost:63342index.html');
                window.location.assign("/mywallet/views/user/dashboard.html");
                callback("true");
            }
            else{
                window.alert(JSON.stringify(response, undefined, 2));
        ...
        doLoginVerification(email, pwd, function (message) {
            if(message === "true"){
                ...
                window.alert('Something went wrong, please try again!');
                 window.location.assign("/mywallet/views/user/login.html");
            }
        })
    }
}
